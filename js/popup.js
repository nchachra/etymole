var ATPopup = {
  ...
  searchAndRender: function(e) {
    if(e.which == 13) {
      ...
      if (query.split(/\s+/).length > 1) {
        ATPopup.flashError("Enter only one word");
      } else {
        query = query.trim();
        ATPopup.displayLastSearch();
        ATPopup.background.EMBg.lastSearch = query;

        ATPopup.sendXhr(query).then(function(response) {
          ATPopup.renderResponse(response);
        }, function() {
          ATPopup.flashError("Failed to get results.");
        });
      }
    }
  },
  ...
  displayLastSearch: function() {
    var lastSearch = ATPopup.background.EMBg.lastSearch;
    if (lastSearch) {
  ...
  sendXhr: function(query) {
    return new Promise(function(resolve, reject) {
      ...
      var url = ATPopup.queryUrlPrefix + query;
